...
if [[ "$internal" == true ]]; then
    ...
    elif [[ "$logical_machine" == "macmini" ]]; then
        queue=OSX.13.Amd64.Perf
...
if [[ "$run_from_perf_repo" == true ]]; then
    ...
    if [[ -n "$perf_fork" ]]; then
        ...
        git clone --branch DiagRestore --depth 1 --quiet https://github.com/DrewScoggins/performance-2 $performance_directory
    fi
